...
const techStacks = [
  ...
  { icon: "github.svg", language: "GitHub Actions" },
  ...
  { icon: "groq.svg", language: "Groq AI" },
];
...
export default function FullWidthTabs() {
  ...
  // ✅ IMPORTANT: certificates will still load even if projects fails
  const fetchData = useCallback(async () => {
    try {
      const [projectsResult, certificatesResult] = await Promise.allSettled([
        supabase.from("projects").select("*").order("id", { ascending: true }),
        supabase.from("certificates").select("*").order("id", { ascending: true }),
      ]);

      // Handle projects (do NOT throw — we don’t want to block certificates)
      if (projectsResult.status === "fulfilled") {
        const { data, error } = projectsResult.value;
        if (error) {
        ...
        console.warn("Projects request failed:", projectsResult.reason);
      }

      // Handle certificates
      if (certificatesResult.status === "fulfilled") {
        const { data, error } = certificatesResult.value;
        if (error) {
        ...
        console.error("Certificates request failed:", certificatesResult.reason);
      }
    } catch (error) {
  ...
  useEffect(() => {
    // ✅ load cache independently (don’t require both)

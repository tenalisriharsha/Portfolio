...
function pickTechIcon(techRaw) {
  const t = String(techRaw || "").toLowerCase();
  if (t.includes("react")) return TECH_ICONS.react;
  if (t.includes("tailwind")) return TECH_ICONS.tailwind;
  if (t.includes("express") || t.includes("flask") || t.includes("django")) return TECH_ICONS.express;
  if (t.includes("python")) return TECH_ICONS.python;
  if (t.includes("javascript") || t.includes("js")) return TECH_ICONS.javascript;
  if (t.includes("html")) return TECH_ICONS.html;
  if (t.includes("css")) return TECH_ICONS.css;
  return TECH_ICONS.default;
}

const TechBadge = ({ tech }) => {
  const Icon = pickTechIcon(tech);
...
const ProjectStats = ({ techStackCount, featuresCount }) => {
  return (
...
const handleGithubClick = (githubLink) => {
  ...
  if (githubLink === "Private") {
    Swal.fire({
      ...
      text: "This project's GitHub repository is private.",
...
const ProjectDetails = () => {
  ...
  const techList = useMemo(() => {
    if (!project) return [];
    // Supports multiple column styles (safe fallback)
    return (
      project.tech_stack ||
      project.TechStack ||
  ...
  return (
    <div className="min-h-screen bg-[#030014] px-[2%] sm:px-0 relative overflow-hidden">
              ...
              <div className="flex flex-wrap gap-3 md:gap-4">
                {githubUrl ? (
                  <a
                    ...
                    onClick={(e) => !handleGithubClick(githubUrl) && e.preventDefault()}
                  >
                    <div className="absolute inset-0 translate-y-[100%] bg-gradient-to-r from-purple-600/10 to-pink-600/10 transition-transform duration-300 group-hover:translate-y-[0%]" />
                    <Github className="relative w-4 h-4 md:w-5 md:h-5 group-hover:rotate-12 transition-transform" />
                    <span className="relative font-medium">GitHub Repository</span>
                  </a>
                ) : (
              ...
              <div className="space-y-4 md:space-y-6">
                ...
                {techList.length > 0 ? (
                  ...
                  <p className="text-sm md:text-base text-gray-400 opacity-50">
                    No technologies added (fill tech_stack in Supabase).
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="bg-white/[0.02] backdrop-blur-xl rounded-2xl p-8 border border-white/10 space-y-6 hover:border-white/20 transition-colors duration-300 group">
                ...
                {featureList.length > 0 ? (
                  ...
                  <p className="text-gray-400 opacity-50">
                    No features added (fill features in Supabase).
                  </p>
                )}
              </div>
            </div>
      ...
      {/* Removed `jsx` prop to avoid the React warning */}
      <style>{`
        @keyframes blob {
